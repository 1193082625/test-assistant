...
> 状态：当前权威版本规划  
> 基线：`v0.7.0` 本地发布验收已完成，等待远端 Ubuntu/macOS CI
> 制定日期：2026-08-04
...
`v0.7.0` 已完成大型仓库性能/内存基线、`.autotest` schema v2 内存迁移与显式事务迁移、安全历史清理，以及 base/LLM/quality 可选依赖拆分。本地完整发布门、三种干净 wheel 安装和真实项目只读验收均已通过；远端 Ubuntu/macOS CI 矩阵通过后才允许创建 Tag。
