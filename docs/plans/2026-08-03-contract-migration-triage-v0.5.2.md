...
6. fixture 使用真实临时 Git 提交表示迁移前后状态，不 mock Git 输出。
7. 运行 `poetry run pytest tests/test_real_project_fixtures.py -q`，保留预期失败作为红灯。
8. 提交 fixtures：`test: add contract migration triage fixtures`。
...
8. 对动态表达式、多个候选目标和无法解析的字符串返回无证据，不猜测。
9. 运行 `poetry run pytest tests/test_contract_migration_analyzer.py -q`。
10. 提交：`feat: extract contract mismatches from failing tests`。
...
7. 同一字段出现类型冲突、可空性冲突、枚举来源冲突、环境覆盖值或无法静态求值时标记 `conflict`。
8. 禁止 import 或执行目标项目模块，避免导入副作用。
9. 运行 `poetry run pytest tests/test_current_contract.py -q`。
10. 提交：`feat: validate current static contract consistency`。
...
9. 满足全部条件时输出 `TEST_DEFECT / HIGH`；缺少 Git 时为 `INCONCLUSIVE / LOW`；当前契约冲突时为 `INCONCLUSIVE / LOW` 并请求确认。
10. 保持 `INFRA_DEFECT`、`FLAKY`、collection error 和已存在的符号删除优先级不回归。
11. 运行 `poetry run pytest tests/test_triage_contract_migration.py tests/test_triage_workflow.py -q`。
12. 提交：`feat: attribute confirmed contract migrations`。
...
2. 用六个真实 fixture 验证 `TEST_DEFECT / HIGH` 和符合契约边界的根因聚类。
3. 用反例验证实现与 Schema 冲突时不能高置信度归因。
4. 运行 `poetry run pytest -q`，预期全部通过。
5. 运行 `poetry build` 与 `git diff --check`。
6. 在干净 Python 3.13 环境安装 wheel，运行六个 fixture smoke。
7. 在 `fitstyle-backend` 上只读复验六个真实案例。
8. 提交：`release: prepare test-assistant v0.5.2`。

## v0.5.2 完成标准

- 六个真实案例无需人工 Git 命令即可得到正确归因。
